...
<img width="532" alt="스크린샷 2021-04-05 오전 1 41 53" src="https://user-images.githubusercontent.com/52665200/113515545-1c820d80-95b0-11eb-9a9b-3d824adfb70a.png">

## 4. OSI 7계층별 이해하기
<img width="618" alt="스크린샷 2021-04-05 오전 1 46 32" src="https://user-images.githubusercontent.com/52665200/113515658-c3ff4000-95b0-11eb-8f1f-6c6797049e7a.png">
...
## 5. 인캡슐레이션과 디캡슐레이션
* 인캡슐레이션
  * 하위 계층에서 데이터를 전기 신호 형태로 바꾸어 네트워크를 통해 보내는 과정
  * 데이터를 데이터 플로 계층(1~4계층)으로 내려보내면서 패킷에 데이터를 넣을 수 있도록 분할
  * 4계층부터 네트워크 전송에 필요한 정보를 헤더에 붙여 넣는다.
  * 4계층, 3계층, 2계층에서 차례로 미리 정의된 비트 단위(0,1)로 구성된 헤더가 추가된다.

* 디캡슐레이션
  * 받은 전기 신호를 데이터 형태로 만들어 2계층으로 보낸다.
    * 2계층 헤더에 포함된 정보를 확인하여 목적지가 자기 자신이 아니라면 패킷을 버린다.
    * 목적지가 맞다면 3계층으로 올려 보내고, 이 과정을 4계층까지 반복한다.
...
* 헤더
  * 헤더가 반드시 포함해야 하는 데이터
    ...
    * 상위 프로토콜 지시자
  * 현재 계층에서 정의하는 정보
    * 4계층 : 데이터의 순서를 정하고, 받은 패킷의 순서가 맞는지, 빠진 패킷은 없는지 점검에 필요한 정보
      * TCP : 시퀀스, 애크 번호 필드
    * 3계층 : 출발지, 도착지 IP 주소
    * 2계층 : 출발지, 도착지 MAC 주소
  * 상위 프로토콜 지시자
    * 디캡슐레이션하는 목적지 쪽에서는 헤더에 아무 정보가 없으면 어떤 상위 프로토콜로 올려보내 주어야 할지 결정할 수 없다.
    * 따라서, 이를 방지하기 위해 인캡슐레이션하는 쪽에서 헤더에 상위 프로토콜 지시자 정보를 포함해야 한다.
    * 4계층 : 포트 번호 (4계층 헤더에 적힌 정보지만, 애플리케이션 계층에서 프로토콜 종류를 나타내주는 정보)
    * 3계층 : 프로토콜 번호
    * 2계층 : 이더 타입
