...
2) 객체지향 언어이다.
3) JVM에서 ‘가비지 컬렉터’라 불리는 데몬 쓰레드에 의해서 자동으로 메모리 관리를 해준다.
4) 오픈 소스
5) 스레드 생성 및 제어와 관련된 라이브러리 API를 제공하고 있기 때문에, 쉽게 멀티 스레드를 구현할 수 있다.
6) 애플리케이션을 실핼될 때 모든 객체가 생성되지 않고, 각 객체가 필요한 시점에 클래스를 동적 로딩해서 생성한다. 또한 유지보수시에는 해당 클래스만 수정하면 되기 때문에 전체 애플리케이션을 다시 컴파일할 필요가 없다. 즉, 유지보수가 편리하다.
- 단점
1) 속도가 느리다
